...
def k_nearest_neighbors_plot(k):
    neighbors = NearestNeighbors(n_neighbors=k)
    neighbors.fit(X_scaled)
    distances, indices = neighbors.kneighbors(X_scaled)
    avg_distances = np.asarray([np.average(distances[i][1:]) for i in range(distances.shape[0])])
    ...
    for i in range(len(sorted_distances)):
        ...
        dist = np.abs((point_end[1] - point_start[1]) * x0 - (point_end[0] - point_start[0]) * y0 +
                      point_end[0] * point_start[1] - point_end[1] * point_start[0]) / \
               np.sqrt((point_end[1] - point_start[1]) ** 2 + (point_end[0] - point_start[0]) ** 2)
...
def dbscan_grid_search():
    ...
    for eps in np.arange(0.1, 2.0, 0.1):
        print("eps", eps)
        for min_samples in range(1, len(X_scaled)):
            dbscan_model = cluster.DBSCAN(eps=eps, min_samples=min_samples)
    ...
    t_end = time.time()
    print(f'Best DBSCAN clustering: eps={best_eps}, min_samples={best_min_samples}, silhouette={best_silhouette}')
    print(f"Grid search execution time: {round((t_end - t_start)*1000, 2)} ms")
    return best_eps, best_min_samples, best_silhouette

# Example usage:
# inflection_min_samples, inflection_eps = k_nearest_neighbors_plot(50)
# plot_dbscan_result(inflection_eps, inflection_min_samples)

optimal_eps, optimal_min_samples, optimal_silhouette = dbscan_grid_search()
plot_dbscan_result(optimal_eps, optimal_min_samples)